...
final class CPRO_Service_MailWizz extends CPRO_Service {
	...
	public function get_api( $credentials ) {
		...
		if ( class_exists( 'MailWizzApi_Autoloader' ) ) {
			$this->api_url = trailingslashit( $credentials['api_url'] ) . 'api/index.php';
	...
	private function render_list_field( $lists, $settings ) {
		...
		if ( isset( $settings['isEdit'] ) && $settings['isEdit'] ) {
			$default = ( isset( $settings['default'] ) ) ? ( ( isset( $settings['default']['mailwizz_lists'] ) ) ? $settings['default']['mailwizz_lists'] : '' ) : '';
		}
		ob_start();

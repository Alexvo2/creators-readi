...
if ( ! class_exists( 'BSF_Target_Rule_Fields' ) ) {
	...
	class BSF_Target_Rule_Fields {
		...
		public function admin_styles( $hook ) {
			...
			wp_enqueue_script(
				'bsf-target-rule', plugins_url( '/', __FILE__ ) . 'target-rule.js', array(
					'jquery',
					'bsf-target-rule-select2',
				), null, true
			);
			wp_enqueue_script(
				'bsf-user-role', plugins_url( '/', __FILE__ ) . 'user-role.js', array(
					'jquery',
				), null, true
			);
		...
		public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
			...
			if ( 'display' == $type ) {
				...
				$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'wp-schema-pro' ) . '</a>';
				$output .= '</div>';
			}

...
if ( ! class_exists( 'BSF_AIOSRS_Pro_Admin' ) ) {
	...
	class BSF_AIOSRS_Pro_Admin {
		...
		public function __construct() {
			...
			add_action( 'plugin_action_links_' . BSF_AIOSRS_PRO_BASE, array( $this, 'license_link' ) );

			add_filter( 'bsf_product_activation_notice', array( $this, 'activation_notice' ), 10, 3 );
			add_filter( 'bsf_registration_page_url_wp-schema-pro', array( $this, 'license_form_link' ), 99 );
		...
		function license_form_link( $link = '' ) {
			return admin_url( 'plugins.php?aiosrs-pro-license-form' );
		}
		 ...
		 * @param  string $url          Activation link.
		 * @param  string $product_name Product name.
		 * @return string
		 */
		public function activation_notice( $message, $url, $product_name ) {

			if ( 'Schema Pro' == $product_name ) {
				$message = __( 'Please', 'wp-schema-pro' ) . ' <a href="' . $url . '&action=settings">' . __( 'activate', 'wp-schema-pro' ) . '</a> ' . __( 'your copy of the', 'wp-schema-pro' ) . ' <i>' . $product_name . '</i> ' . __( 'to get update notifications, access to support features & other resources!', 'wp-schema-pro' );
			}
			return $message;
		}
		...
		function license_link( $links = array() ) {
			...
			$status         = 'inactive';
			$license_string = __( 'Activate License', 'wp-schema-pro' );
			if ( function_exists( 'bsf_extract_product_id' ) ) {
				$product_id = bsf_extract_product_id( BSF_AIOSRS_PRO_DIR );

				if ( class_exists( 'BSF_License_Manager' ) ) {
					if ( BSF_License_Manager::bsf_is_active_license( $product_id ) ) {
						$status         = 'active';
						$license_string = __( 'License', 'wp-schema-pro' );
					}
				}
			}

			$action_links = array(
				'license' => '<a class=" ' . esc_attr( $status ) . '" id="aiosrs-pro-license-form-btn" href="#" aria-label="' . esc_attr( $license_string ) . '">' . esc_html( $license_string ) . '</a>',
			);
